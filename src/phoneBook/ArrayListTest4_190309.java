...
public class ArrayListTest4_190309 {
	...
	private static void addPhoneList(String string, String string2) {
		...
		//1 파라미터의 값을 이용해 phonelist에 내가 추가하려는 정보가 있는지 확인후
		//이름만 같으면 1  전화번호까지 같으면 2를 리턴으로 받는다
		int chk = chkDouble(string, string2);
		if(chk == 1) {
			System.out.println("중복된 이름이 존재합니다");
		}else if(chk == 2) {
			System.out.println("중복된 이름 그리고 전화번호가 존재합니다");
		}else {
	...
	//목적: 이름, 이름 그리고 전화번호 중복된 케이스 chk 변수에 넣기
	private static int chkDouble(String string, String string2) {
		// TODO Auto-generated method stub
		
		int chk = 0;
		
		//1-1 이름과 전화번호를 체크하여 chk 값을 1,2,0 으로 확정하기
		for(PhoneItem item : phonelist) {
			if(string.equals(item.getName())) {
				chk = 1;
			}
			if(string.equals(item.getName()) && string2.equals(item.getTelNum())) {
				chk = 2;
			}
		}
		return chk;
...
//					System.out.println("strArray 값내 세번째 값 확인: "+strArray[2]);	//데이터가 없으므로 ArrayIndexOutOfBoundsException 발생 
	...
	public static void main(String[] args) {
		...
		printPhoneList();
		addPhoneList("tel6","01066666667");			//새로운 연락처 추가	
